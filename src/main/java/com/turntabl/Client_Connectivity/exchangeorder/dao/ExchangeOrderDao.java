...
public interface ExchangeOrderDao extends JpaRepository<ExchangeOrder,Integer> {
    ...
    @Query("select e from ExchangeOrder e where e.order.id = :client_order_id")
    ExchangeOrder findbyClientOrderId(@Param("client_order_id") int client_order_id);

    @Modifying(clearAutomatically = true)
    @Query("update ExchangeOrder e set e.orderKey = :order_key where e.order.id = :client_order_id")
